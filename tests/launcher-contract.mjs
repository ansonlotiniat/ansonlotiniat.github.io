...
async function settle(page) {
    await page.waitForFunction(() => document.body.classList.contains("is-ready")
        && (!window.gsap || window.gsap.getTweensOf(document.querySelectorAll(
...
async function inspectPlates(page, selector, label, expectedCount, horizontal = false) {
    const records = await page.locator(selector).evaluateAll((plates) => plates.map((plate) => {
        ...
        return {
            ...
            radius: style.borderRadius, mask: style.maskImage,
    ...
    for (const [index, record] of records.entries()) {
        ...
        assert(record.radius === first.radius && record.mask === first.mask && record.shadow === first.shadow, `${label}: inconsistent mask/shadow`);
        if (horizontal) {
...
async function verifyPixelMasks(page, PNG, label, selector = ".dock .launcher-plate") {
    ...
    try {
        ...
        for (let index = 0; index < await plates.count(); index += 1) {
            ...
            if (!reference) { reference = image; continue; }
            assert(image.width === reference.width && image.height === reference.height, `${label}: raster dimensions differ`);
            let mismatches = 0;
            ...
            for (let pixel = 3; pixel < image.data.length; pixel += 4) {
                ...
                maximumAlphaDelta = Math.max(maximumAlphaDelta, Math.abs(image.data[pixel] - reference.data[pixel]));
        ...
        return {rasterWidth: reference.width, rasterHeight: reference.height, silhouetteMismatches: 0, largestAlphaRounding};
    } finally {
...
export async function verifyLauncherContract(browser, url, label, PNG) {
    ...
    for (const dpr of [1, 2]) {
        const context = await browser.newContext({viewport: {width: 1459, height: 900}, deviceScaleFactor: dpr});
        try {
            ...
            for (const width of [1459, 1440, 1000, 760, 520, 480, 375, 360]) {
                ...
                const record = await inspectPlates(page, ".dock .launcher-plate", `${label} ${width}px DPR${dpr}`, 10, true);
                if (width >= 1001) {
            ...
            if (dpr === 2) await page.locator(".dock").screenshot({path: path.join(output, `launcher-${label}-dock.png`)});
            results.push({label: `${label} DPR${dpr} raster`, ...await verifyPixelMasks(page, PNG, label)});

            await page.getByRole("button", {name: "Apps", exact: true}).click();
            await settle(page);
            results.push(await inspectPlates(page, ".launchpad-icon .launcher-plate", `${label} Apps DPR${dpr}`, 8));
            results.push({label: `${label} Apps DPR${dpr} raster`, ...await verifyPixelMasks(page, PNG, label, ".launchpad-icon .launcher-plate")});
            if (dpr === 2) await page.screenshot({path: path.join(output, `launcher-${label}-apps.png`)});
            await page.setViewportSize({width: 480, height: 900});
            results.push(await inspectPlates(page, ".launchpad-icon .launcher-plate", `${label} mobile Apps DPR${dpr}`, 8));
            await page.setViewportSize({width: 1459, height: 900});
            await page.keyboard.press("Escape");
            await page.locator("#launchpad").waitFor({state: "hidden"});
            await page.getByRole("button", {name: "Open Explore", exact: true}).click();
            await settle(page);
            results.push(await inspectPlates(page, ".app-icon .launcher-plate", `${label} Spotlight DPR${dpr}`, 8));
            results.push({label: `${label} Spotlight DPR${dpr} raster`, ...await verifyPixelMasks(page, PNG, label, ".app-icon .launcher-plate")});
        } finally {
            await context.close();
        }
    }
    fs.writeFileSync(path.join(output, `launcher-${label}-measurements.json`), JSON.stringify(results, null, 2) + "\n");
