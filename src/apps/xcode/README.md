...
- 入口：`#xcode`；名稱／排序／圖示在 [manifest](manifest.json)。
- 畫面：[view](view.html)；互動：[controller](controller.js)。sports/debate tab、before/during/after 階段及角色切換。
- 樣式：[styles](styles.css) 是內部畫面，[window](window.css) 是 native chrome／視窗與響應式。
- 狀態與限制：controller.js 狀態只在記憶體；畫面中的 source 是產品邏輯展示。
