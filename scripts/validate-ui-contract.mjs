...
const style = read("src/shell/styles/base.css");
const macos = read("src/shell/styles/theme.css");
const dock = read("src/shell/styles/dock.css");
const music = read("src/apps/music/window.css");
const icons = read("src/shell/styles/icons.css");
...
for (const match of music.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    ...
    if (/\.music-player-actions\b/.test(selector)) {
        check(
            !/margin-(?:left|right)\s*:/.test(declarations),
            `Music action uses a margin chain: ${selector.trim()}`,
        );
    }
}
...
check(
    ...
    "Music controls contain duplicate data-music-control values",
);
...
check(
    !apps.includes('className: "music-icon"'),
    "The Apple Music App icon collides with the Music SVG class",
);
check(
    main.includes('plate.className = "launcher-plate"'),
    "Launchers are missing their shared artwork plate",
);
check(
    main.includes("appIcon(shellIcons.apps") && main.includes("appIcon(shellIcons.mail"),
    "Fixed launchers bypass shared icon rendering",
);
...
for (const [file, css] of [
    ...
    check(
        !/\.(?:netflix|overleaf)-icon(?:\s|:|\{)/.test(css),
        `${file} contains a retired per-icon geometry override`,
    );
}
...
for (const asset of ["style.css", "macos.css", "dock.css", "icons.css", "apps.config.js", "main.js"]) {
    const escaped = asset.replace(".", "\\.");
    const version = index.match(new RegExp(`(?:href|src)="${escaped}\\?v=([^"]+)"`))?.[1];
    check(
        version === contractVersion,
        `${asset} cache version ${version || "missing"} does not match ${contractVersion || "the contract"}`,
    );
}
...
console.log(
    "UI contract passed: Dock and Music ownership, structure, semantics, and cache versions are consistent.",
);
